    ...
    for feature in features_array:
        ...
        for marker in markers:
            ...
            if delta_type == "removed":
                print("markerid removed = " + str(marker[0]))
...
'''Get feature's markers'''
def getDeltas(features_array, queryRemoved, queryAdded):
    for feature in features_array:
        ...
        try:
            # print(feature[0])
            cur.execute(queryRemoved, (feature[0],))
        except psycopg2.Error as e:
            print("\nERROR: UNABLE TO GET MARKERS [" + delta + "]: " + e) 
            exit()
        ...
        try:
            # print(feature[0])
            cur.execute(queryAdded, (feature[0],))
        except psycopg2.Error as e:
            print("\nERROR: UNABLE TO GET MARKERS [" + delta + "]: " + e) 
            exit()
...
query_Removed = "SELECT id, latitude, longitude, address, last_timestamp, precision, total_detections, imagepath \
                FROM marker \
                WHERE featureid = %s \
...
query_Added = "SELECT id, latitude, longitude, address, last_timestamp, precision, total_detections, imagepath \
                FROM marker \
                WHERE featureid = %s \
                AND first_timestamp > CURRENT_DATE - INTERVAL '1 month';"

getDeltas(features, query_Removed, query_Added)
